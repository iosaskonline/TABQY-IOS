...
-(void)startServiceToGetTodySpl
{
    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    
    [self performSelectorInBackground:@selector(serviceToGetTodySpl) withObject:nil];
    
    
}

-(void)serviceToGetTodySpl
{
    ECSServiceClass * class = [[ECSServiceClass alloc]init];
    [class setServiceMethod:POST];
    NSString *selectedIp=[ECSUserDefault getStringFromUserDefaultForKey:@"ResetIP"];
    if (selectedIp.length) {
        [class setServiceURL:[NSString stringWithFormat:@"http://%@%@todayspecial",selectedIp,SERVERURLPATH]];
    }else{
        [class setServiceURL:[NSString stringWithFormat:@"http://%@%@todayspecial",@"tabqy.com",SERVERURLPATH]];
    }
    //[class setServiceURL:[NSString stringWithFormat:@"%@todayspecial",SERVERURLPATH]];
    ...
    [class addJson:dict];
    [class setCallback:@selector(callBackServiceToGetTodySpl:)];
    [class setController:self];
    
    [class runService];
}

-(void)callBackServiceToGetTodySpl:(ECSResponse *)response
{
    ...
    {
        self.arrayFood=[[NSMutableArray alloc]init];
        NSArray *arr=[rootDictionary valueForKey:@"todayspecial"];
