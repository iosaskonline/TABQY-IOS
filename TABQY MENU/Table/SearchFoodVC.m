...
@property(weak,nonatomic)IBOutlet UIImageView *imgHotDeal;
@property(weak,nonatomic)IBOutlet UILabel *lblnameHotDeal;
@property(weak,nonatomic)IBOutlet UILabel *lblDisHotDeal;
@property(weak,nonatomic)IBOutlet UILabel *lblPriceHotDeal;
...
-(void)callBackServiceToHotDeal:(ECSResponse *)response
{
    ...
    if(response.isValid)
    {
       ...
       // NSString *str=[dictionary objectForKey:@"food_image"];
        if (dictionary) {
           ...
           // NSString *imgurl=[NSString stringWithFormat:@"%@%@",FOODIMAGE,object.foodImage];
            ...
            [self.imgHotDeal ecs_setImageWithURL:[NSURL URLWithString:imgurl] placeholderImage:nil options:0 completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL)
             {
                  [self.indicator stopAnimating];
                 self.indicator.hidden=YES;
             }];
            //self.imgHotDeal.contentMode = UIViewContentModeScaleAspectFit;

            self.lblnameHotDeal.text=object.foodName;
            self.lblDisHotDeal.text=object.foodDescription;
            self.lblPriceHotDeal.text=[NSString stringWithFormat:@"%@ %@",self.appUserObject.resturantCurrency,object.price];
...
-(void)serviceToGetSearchFood
{
    ...
    NSString *selectedIp=[ECSUserDefault getStringFromUserDefaultForKey:@"ResetIP"];
    if (selectedIp.length) {
        [class setServiceURL:[NSString stringWithFormat:@"http://%@%@foodsearch",selectedIp,SERVERURLPATH]];
    }else{
        [class setServiceURL:[NSString stringWithFormat:@"http://%@%@foodsearch",@"tabqy.com",SERVERURLPATH]];
    }
